...
if __name__ == "__main__":
    ...
    #     2018, 1, 1), todate=datetime(2020, 1, 1))
    df = ts.get_k_data('600519', autype='qfq', start='2018-01-01')
    ...
    print(cerebro.broker.getvalue(), '执行前')
    back = cerebro.run()
    print(cerebro.broker.getvalue(), '执行后')
    ...
    # 结果转成dataframe
    par_df = pd.DataFrame(
        par_list, columns=['length_fast', 'length_slow', 'return', 'dd', 'sharpe'])
